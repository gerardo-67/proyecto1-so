...
void* decodificar_archivo(FILE* archivo_entrada, long posicion_inicio, int indice_archivo) {
    
    
    // Posicionarse en el inicio de este archivo
    fseek(archivo_entrada, posicion_inicio, SEEK_SET);
    
    // Obtener el largo del nombre
    unsigned char nombre_len;
    fread(&nombre_len, 1, 1, archivo_entrada);
    
    // Leer el nombre del archivo
    char nombre[256] = {0};
    fread(nombre, 1, nombre_len, archivo_entrada);
    nombre[nombre_len] = '\0';
    
    // Leer el total de bits
    unsigned int total_bits;
    fread(&total_bits, sizeof(unsigned int), 1, archivo_entrada);
    
    // Leer la tabla de códigos
    HuffmanCode tabla[MAX_CODIGOS];
    int total_codigos = leer_tabla(archivo_entrada, tabla);
    
    // Crear un buffer para leer todos los datos de una vez
    long posicion_actual = ftell(archivo_entrada);
    int bytes_datos = (total_bits + 7) / 8;
    ...
    fread(buffer_datos, 1, bytes_datos, archivo_entrada);
    ...
    printf("Archivo #%d descomprimido: %s (%u bits)\n", 
           indice_archivo, nombre, total_bits);
...
int main() {
    ...
    // Iniciar los procesos con fork()
    for (int i = 0; i < cantidad_archivos; i++) {
        // Crear un proceso para cada archivo
        int pid = fork();
        if (pid == 0) {
            FILE* f_hijo = fopen("comprimido.bin", "rb");
            if (!f_hijo) exit(1);
            decodificar_archivo(f_hijo, posiciones[i], i);
            exit(0);  // Termina el hilo hijo
        } else if (pid < 0) {
            printf("Error creando proceso hijo %d\n", i);
        }
    }
